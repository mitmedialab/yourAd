...
var channels = new ChannelYourAd();
...
function getAccomplishments(){
   return storage_get("accomplishments") || [];
}

function clickAd(args) {

    //update accomplishments
    var accomplishments = storage_get("accomplishments") || [];
    args.timestamp = Date.now();
    accomplishments.push(args);
    storage_set("accomplishments", accomplishments);

    //send a message to options page that it should refresh
    chrome.runtime.sendMessage({command:"accomplishment-update"});

    //tell channels object that this ad was clicked so it can update which
    //ads should be displayable
    return channels.clickAd(args);
}
